...
// pattern: `git rev-parse --short=10 HEAD` with `dev` fallback so fresh
// clones without git history and offline builds still configure).
...
android {
    ...
    defaultConfig {
        ...
        // (forgerig pattern with `dev` fallback).
